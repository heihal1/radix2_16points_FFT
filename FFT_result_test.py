...
def fft_without_calling_function(input_real, input_imag):
    N = int(16)  # 16点fft
    ...
    for k in range(8):
        tf_real.append((math.cos(-2 * math.pi * k / N)))
        tf_imag.append((math.sin(-2 * math.pi * k / N)))

    for s in range(stages): # N = 16，故总共分为四级计算
        ...
        for i in range(0, N, step):
            for j in range(butterfly_calculation_span):
                ...
                c_real = (b_real * W_real - b_imag * W_imag)
                c_imag = (b_real * W_imag + b_imag * W_real)
...
if __name__ == "__main__":
    ...
    data_real, data_imag = fft_without_calling_function(input_real, input_imag)
    ...
    # 不调用函数结果
    print("fft_without_calling_function output= ")
    for i in range(16):
        print(f"X[{i}] = {data_real[i]} + j{data_imag[i]}")

    # 调用函数结果
    print("fft_calling_function output= ")
    for i in range(16):
